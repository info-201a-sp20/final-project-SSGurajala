...
intro_page <- tabPanel(
  ...
  mainPanel(
    h1("Project Overview"),
    p("The acute nature of infectious disease often draws our
      attention because of the imminent pain and suffering they
      bring to millions around the globe, as seen by the current
      Coronavirus pandemic. However, the physical and mental impact
      of non-infectious chronic diseases can also be overwhelming
      to patients, and should not be ignored. In our project, we
      focused primarily on how chronic non-infectious diseases have
      infected or continue to infect the lives of people around the
      world. So we picked 3 diseases: cardiovascular disease,
      respitory disease, and neoplasm, and raised three questions
      that we are interested in exploring accordingly. The
      disability-adjusted life year (DALY) is our primary metric
      for disease burden, and factors in years lost to ill-health,
      disability or early death. This data was taken from the GHDx
      data selecting tool for the 2017 IHME Global Burden of
      Disease Study."),
    tags$a(
      id = "source", href = "http://ghdx.healthdata.org/gbd-results-tool",
...
first_page <- tabPanel(
  ...
  h2("How does income level/financial stability track with disease
     burden globally?"),
  sidebarPanel(
    h4("Input:"),
...
second_page <- tabPanel(
  ...
  h2("How does disease burden differ between cardiovascular disease,
...
summary_page <- tabPanel(
  ...
  mainPanel(
    ...
    h5(strong("How does income level/financial stability track with
              disease burden globally?")),
    h5(strong("How does disease burden differ between cardiovascular disease,
              chronic respiratory disease, and neoplasms globally?")),
    h5(strong("How does disease burden for cardiovascular disease,
              chronic respiratory disease, and neoplasms change
              over time?")),
    br(),
